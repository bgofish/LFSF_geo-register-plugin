...
class MainPanel(lf.ui.Panel):
    ...
    def _draw_lla_section(self, layout, scale, theme):
        ...
        if utm:
            ...
            layout.text_colored(f"Easting  : {utm['easting']:.3f} m",  theme.palette.text_dim)
            layout.text_colored(f"Northing : {utm['northing']:.3f} m", theme.palette.text_dim)
            layout.text_colored(f"RL (alt) : {utm['alt_m']:.3f} m",    theme.palette.text_dim)
            layout.text_colored(
                f"{utm['epsg']} {utm['easting']:.2f} m E  "
                f"{utm['northing']:.2f} m N  {utm['alt_m']:.0f} m RL",
                theme.palette.text_dim
            )
